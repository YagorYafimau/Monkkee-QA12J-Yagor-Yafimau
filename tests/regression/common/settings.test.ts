...
test('Test Settings Page', async ({ page }) => {
  ...
  const languageText = await page.locator('body').textContent();
  ...
  const languageTextEn = await page.locator('body').textContent();
  ...
  const lightThemeText = await page.locator('body').textContent();
  expect(lightThemeText?.trim()).toContain('Light');

  await settingsPage.changeColourScheme('dark');
  await settingsPage.waitForNotification('Your colour scheme has been changed');
  const darkThemeText = await page.locator('body').textContent();
  ...
  const timeoutTextMinus1 = await page.locator('body').textContent();
  ...
  const timeoutText10 = await page.locator('body').textContent();
